...
ABOUT = {
    ...
    "test_amd_upscale_mv_knob.py": "the upscale's motion vectors are the shipped fix, gated on the private copy so they can never drive the runtime's re-create loop",
    ...
    "test_amd_upscale_private_arm.py": "the upscale runs on a private copy of the upscaler by default, same file and name, and a copy that failed to load degrades to the old picture",
    ...
    "test_probe_launcher.py": "each probe launcher ships as a one-click file, sets exactly its own variable to exactly its own value in the launched process",
