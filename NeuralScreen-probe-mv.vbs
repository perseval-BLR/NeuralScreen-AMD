' NeuralScreen-probe-mv.vbs - the per-frame probe WITH the upscale motion-vector arm
' (NS_AMD_PROBE_EACH=1 and NS_AMD_UPSCALE_MV=1)
' Same as NeuralScreen-probe.vbs, and in addition the FSR upscale dispatch is
' handed a motion-vector surface instead of none.
'
' Why: the upscale's output goes wrong on every other frame while its input
' is flat, on two different Radeon generations. The motion vectors are the one
' input of that dispatch the FidelityFX API does not mark optional, and it has
' always been given none. This launcher is that one change, and the probe is on
' so the run says whether it helped. It is a test, not a fix.
'
' Run it ONCE after a run with NeuralScreen-probe.vbs, same window and same
' work scale, so the two logs differ in exactly this. Slow picture, like the
' other probe launcher: a diagnosis, not a way to play.
