...
rem The FidelityFX bridge: the dispatch the neural runtime follows. Own unit
rem for the same reason - it compiles standalone (amd\build-check-fsr.bat).
cl /nologo /O2 /EHsc /W3 /MD /std:c++17 /Iinclude /c amd\amd_fsr.cpp
if errorlevel 1 exit /b 1

cl /nologo /O2 /EHsc /W3 /MD /std:c++17 /Iinclude /Isrc dlss5-feed-host64.cpp spout_bridge.cpp amd_runtime.obj amd_fsr.obj ^
   /Fe:nvngx.dll ^
